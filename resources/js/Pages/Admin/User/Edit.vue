    ...
    <div class="max-w-3xl bg-white rounded-md border border-gray-300 overflow-hidden">
        <form @submit.prevent="update">
            <div class="p-8 space-y-12">
                <div class="border-b border-gray-900/10 pb-12">
                    ...
                    <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 md:grid-cols-2">
                        ...
                        <text-input v-model="form.email" type="email" :error="form.errors.email" label="Email" required/>
                        <text-input v-model="form.contact_number" :error="form.errors.contact_number" label="Contact Number" required/>
                        <date-input v-model="form.birthdate" :error="form.errors.birthdate" label="Birthdate" required/>
                    </div>
                </div>

                <div class="border-b border-gray-900/10 pb-12">
                    <h2 class="text-base font-semibold leading-7 text-gray-900">Identity Details</h2>
                    <p class="mt-1 text-sm leading-6 text-gray-600">This information is only visible to admin.</p>

                    <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 md:grid-cols-2">
                        <select-input v-model="form.id_type_id" :error="form.errors.id_type_id" label="Identification Type" required>
                            <option disabled value="">Please select identification type</option>
                            <option v-for="id_type in idTypes" :value="id_type.id">{{id_type.name}}</option>
                        </select-input>
                        <text-input v-model="form.id_number" :error="form.errors.id_number" label="Identification Number" required/>
                        <text-input v-model="form.employee_id" :error="form.errors.employee_id" label="Employee ID" required/>
                    </div>
                </div>

                <div class="border-b border-gray-900/10 pb-12">
                    <h2 class="text-base font-semibold leading-7 text-gray-900">Assign Role</h2>
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm leading-6 text-gray-600">Turn on the access toggle to allow the employee to access their account.</p>
            ...
            <div class="flex flex-row-reverse space-x-2 space-x-reverse  items-center justify-start px-8 py-4 bg-gray-50 border-t border-gray-100">
                <loading-button :loading="form.processing" :disabled="!form.isDirty" class="btn-primary" type="submit">Update Employee</loading-button>
    ...
    data() {
        return {
            ...
            form: this.$inertia.form({
                ...
                access_expiry_date: this.user.access_expiry_date != null ? this.user.access_expiry_date.substring(0, 10) : '',
            }),
        }
    },
    methods: {
